...
Остальные интеграции живут в `.env.prod`, а не здесь: Claude (`ANTHROPIC_*`), почтовый
ingest (`MAILRU_*`), СБИС (`SBIS_*`) и список отправителей налоговых документов. Старт
они не роняют — выключают функции, чаще всего молча, — поэтому в go-live-набор не входят
и деплой не блокируют. Что именно перестаёт работать без каждой, расписано в
`SECRETS.md`, раздел «Необязательные контуры»; `check-prod-secrets.sh` назовёт пустые
предупреждением.
...
Ожидаемый результат: check проходит; значения секретов не печатаются. Строки `WARN:` —
не отказ: это выключенные контуры и небезопасные умолчания, их надо прочитать и решить,
так ли задумано.
