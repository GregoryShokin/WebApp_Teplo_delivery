...
- **`TELEGRAM_INTAKE_BOT_TOKEN` / `TELEGRAM_INTAKE_ALLOWED_CHAT_IDS`** — приёмка коммунальных
  квитанций из мессенджера. Токен пустой → проход возвращает `not_configured` и в лог не
  пишет; строки на «Странице на оплату» появятся только через кнопку «Загрузить документ».
  Список чатов пустой означает **никого**: имя бота публично, и без белого списка любой
  прохожий заводил бы обязательства в учёте. Свой chat id человек узнаёт от самого бота —
  он отвечает им на первое же сообщение из неразрешённого чата.
