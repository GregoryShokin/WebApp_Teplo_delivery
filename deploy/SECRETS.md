...
- **`TELEGRAM_INTAKE_BOT_TOKEN`** — приёмка коммунальных квитанций из мессенджера. Токен
  пустой → проход возвращает `not_configured` и в лог не пишет; строки на «Странице на оплату»
  появятся только через кнопку «Загрузить документ». Белого списка чатов НЕТ (решение владельца
  02.08.2026): документ принимается от любого, кто написал боту, а отправитель записывается в
  строку приёмки и виден в столбце «от кого» — сомнительную строку исключают одной кнопкой.
  Сообщения старше `TELEGRAM_INTAKE_MAX_MESSAGE_AGE_MINUTES` (по умолчанию час) бот в работу
  не берёт: Телеграм хранит неподтверждённое сутки и отдаёт всё разом при первом обращении,
  так что первый запуск после деплоя иначе завёл бы пачку вчерашних документов.
  **Одного бота нельзя читать двумя процессами.** `getUpdates` отдаёт обновление ровно одному
  потребителю, поэтому старый локальный `integrations/tbank/scripts/payment_order_bot.py`
  (выведен из работы 02.08.2026) с этим токеном запускать нельзя: он начнёт перехватывать
  пересланные документы, и часть их пропадёт молча, без единой ошибки в логах.
