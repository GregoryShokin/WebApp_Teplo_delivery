"""Справочник строк ОПиУ: 53 статьи эталона, 6 добавленных владельцем, 20 расчётных, 3 служебных.
...
ШЕСТЬ СТРОК СВЕРХ ЭТАЛОНА заведены по решению владельца 03.08.2026: «Ремонт оборудования»,
«Аренда офиса», «Содержание офиса», «Обучение персонала», «Реклама на Яндекс-картах» и
«Убыток от выбытия ОС». Статьи ДДС под них давно живут и расход по ним реален — в таблице
владельца они просто не выделены. Альтернативой было впихнуть их в соседние строки, и тогда
отчёт перестал бы отвечать на вопрос «сколько стоит офис» и «работает ли размещение на
Картах». «Зарплата собственника» отдельной строкой НЕ заводится: владелец отнёс её к
«Зарплате администрации», это делается правилом статьи, а не новой строкой.
...
LINES: list[tuple] = [
    ...
    (
        "equipment_repair", "Ремонт оборудования", "overhead", "source",
        315, 1, -1, "cash", False, "active", None, None, None,
        "Текущий ремонт оборудования торговых точек. Своя строка по решению владельца "
        "03.08.2026: в эталоне её нет, но статья ДДС живёт с 0231 и расход реален. "
        "Капитальный ремонт сюда не попадает — он капитализируется в стоимость ОС",
    ),
    (
    ...
    (
        "office_rent", "Аренда офиса", "admin", "source",
        434, 1, -1, "document", False, "active", None, None, None,
        "Аренда офисного помещения. Своя строка по решению владельца 03.08.2026: офис "
        "обслуживает управление, а не торговую точку, поэтому не в общепроизводственных",
    ),
    (
        "office_maintenance", "Содержание офиса", "admin", "source",
        436, 1, -1, "cash", False, "active", None, None, None,
        "Расходники и обслуживание офиса. Своя строка по решению владельца 03.08.2026",
    ),
    (
    ...
    (
        "staff_training", "Обучение персонала", "admin", "source",
        475, 1, -1, "cash", False, "active", None, None, None,
        "Обучение и аттестация сотрудников. Своя строка по решению владельца 03.08.2026; "
        "отделена от «Расходов на персонал», где корпоративы и подарки",
    ),
    (
    ...
    (
        "yandex_maps_ads", "Реклама на Яндекс-картах", "commercial", "source",
        535, 1, -1, "document", False, "active", None, None, None,
        "Приоритетное размещение на Яндекс-картах. Своя строка по решению владельца "
        "03.08.2026: канал отдельный, смешивать с контекстом нельзя — иначе не видно, "
        "что из них работает",
    ),
    (
    ...
    (
        "asset_disposal_loss", "Убыток от выбытия ОС", "below_expense", "source",
        705, 1, -1, "calendar", False, "active", None, None, None,
        "Остаточная стоимость списанных основных средств. Своя строка по решению владельца "
        "03.08.2026: модуль «Учёт ОС» её уже считает и отдаёт вместе с амортизацией, но в "
        "эталоне такой строки нет. Величина неденежная, как и амортизация",
    ),
    (
