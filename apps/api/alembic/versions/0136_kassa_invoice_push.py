"""Право «Отправлять накладные из Кассы в iiko» (kassa.invoices.push).

Отправка накладной в iiko раньше гейтилась только широким counterparties.operate,
которого нет у кассира и которое ему не выдают точечно. Накладные из Кассы
(source=kassa_invoice) тоже должны попадать на склад в iiko, поэтому вводим узкое
право и раздаём его ролям, создающим накладные из Кассы. Бэкенд-гейт push —
operate OR kassa.invoices.push, поэтому управляющий/менеджер не теряют доступ.

Revision ID: 0136_kassa_invoice_push
Revises: 0135_deposit_rbac
...
revision = "0136_kassa_invoice_push"
down_revision = "0135_deposit_rbac"
branch_labels = None
depends_on = None

NEW_CODES = ("kassa.invoices.push",)
