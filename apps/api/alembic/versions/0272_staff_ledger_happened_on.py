...
ведомость: её хозяйственный день — ``payroll_period.payroll_date``, день выплаты. Ручную строку
...
def upgrade() -> None:
    ...
    # Строки ведомости — днём выплаты своего периода.
    for table in ("deposit_transaction", "accumulation_fund_transaction"):
        op.execute(
            f"""
            UPDATE {table} AS t
               SET happened_on = p.payroll_date
              FROM payroll_run r
