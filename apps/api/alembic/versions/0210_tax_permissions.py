"""Налоги: права на модуль.

``accounting.taxes.read`` — смотреть расчёт, сверку и календарь сроков;
``accounting.taxes.manage`` — продвигать документы бухгалтера в обязательства и вносить платежи.

Выдаём владельцу и админу: налоговый контур — зона ответственности собственника
(решение владельца 2026-05-27: финализирует он, готовит отчётность ИИ-агент).

Revision ID: 0210_tax_permissions
Revises: 0209_tax_document_intake
"""
...
revision = "0210_tax_permissions"
down_revision = "0209_tax_document_intake"
branch_labels = None
depends_on = None

NEW_CODES = ("accounting.taxes.read", "accounting.taxes.manage")
