...
def test_month_in_brackets_after_address_is_a_period() -> None:
    """«(Июль 2026 г.)» в конце строки услуги — тоже период.

    Так выставляют ЧОО «Охрана Юг» и СпецАвто Юг: месяц дописан в скобках после адреса
    объекта, без слов «за» и «период». Форма отличается от сокращённой («Авг. 2026»), но
    природа та же — маркера нет, и парсер её пропускал.

    Отдельно проверяем, что дата самого счёта периодом не становится: «от 1 июля 2026 г.»
    стоит в шапке каждого такого документа, и без запрета дня перед месяцем расход уехал бы
    в месяц выставления счёта вместо месяца услуги.
    """
    guard = _extract_service_periods(
        "Оказание охранных услуг на объекте, Черникова ул, дом № 4В, пом 1 (Июль 2026 г.) 1 мес"
    )
    assert [(start, end) for start, end, _, _ in guard] == [
        (date(2026, 7, 1), date(2026, 7, 31))
    ]
    assert _extract_service_periods("Счет на оплату № УТ-4208 от 1 июля 2026 г.") == []


# --- закрывающие документы: собственные реквизиты против реквизитов основания ---------------

IIKO_ACT = """\
Лицензиар
АО "АЙКО", ИНН 1655166016, 117587, Москва г, Варшавское ш, дом 118, тел.: 8(495)2152200, р/с
40702810312010245858, в банке Филиал "Корпоративный" ПАО "Совкомбанк", БИК 044525360,
к/с 30101810445250000360
организация, адрес, телефон, факс, банковские реквизиты
Лицензиат ИП Шокина Кристина Юрьевна, ИНН 890307589201, р/с 40802810100002438573,
в банке АО "ТБанк", БИК 044525974
организация, адрес, телефон, факс, банковские реквизиты
Основание Счет № 040726-40618-лсп от 04.07.2026
договор, заказ-наряд
Номер документа Дата составления
Акт на передачу прав 10826-9432-лсп 01.08.2026
1 Лицензия на ПО Курьерика Курьер БП-00005571 мес 362 1,000 3 540,00 3 540,00 Без НДС 3 540,00
2 Лицензия на ПО Курьерика Логист БП-00005572 мес 362 1,000 720,00 720,00 Без НДС 720,00
Итого 2,000 Х 4 260,00 Х 4 260,00
Всего по акту 2,000 Х 4 260,00 Х 4 260,00
От Лицензиара Генеральный директор Папин Д. Н. От Лицензиата
должность подпись расшифровка подписи должность подпись расшифровка подписи
М.П. "01" августа 2026 года М.П. "     " _____________ 20     года
"""


def test_closing_document_keeps_its_own_number_and_date() -> None:
    """Акт берёт СВОИ номер и дату, а реквизиты счёта-основания уходят в отдельные поля.

    Прод, 04.08.2026: два акта iiko за август встали в реестры как «счёт 040726-40618-лсп от
    04.07.2026» — то есть двойниками уже оплаченных июльских счетов. Причина: собственный
    номер акт печатает таблицей без «№», поэтому поиск «Счёт №…» находил в нём только строку
    «Основание», а «от 04.07.2026» оттуда же становилось датой документа. В реестре, где
    сортировка и глаз идут по дате, августовские документы искать бесполезно — владелец их
    не нашёл и решил, что почта их вовсе не приняла.
    """
    rec = deterministic_recognize(IIKO_ACT)

    assert rec.document_kind == "act"
    assert rec.invoice_number == "10826-9432-лсп"
    assert rec.invoice_date == date(2026, 8, 1)
    # Реквизиты основания сохранены: по ним ingest находит счёт и наследует его период услуги.
    assert rec.basis_number == "040726-40618-лсп"
    assert rec.basis_date == date(2026, 7, 4)


def test_signature_date_is_not_a_service_period() -> None:
    """Дата подписи бланка периодом услуги не становится.

    «М.П. "01" августа 2026 года» обходило запрет «дня перед месяцем»: день стоит в кавычках.
    Акты iiko получали период по дате подписи, и совпадало это с правдой лишь потому, что iiko
    подписывает акт первым числом оплаченного месяца — подпиши он тот же акт 1 сентября, и
    расход августа уехал бы в сентябрь молча.
    """
    assert _extract_service_periods('М.П. "01" августа 2026 года М.П. "     " ___ 20  года') == []
    assert _extract_service_periods('"31" июля 2026 года') == []
    # Период в теле документа при этом по-прежнему читается.
    assert [
        (start, end) for start, end, _, _ in _extract_service_periods("обслуживание Авг. 2026")
    ] == [(date(2026, 8, 1), date(2026, 8, 31))]


def test_invoice_number_search_ignores_basis_reference() -> None:
