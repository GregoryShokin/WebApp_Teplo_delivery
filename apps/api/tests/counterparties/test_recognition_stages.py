...
def test_stages_split_queue_by_who_makes_the_step(
    ...
    # выводится из даты денег — и строка честно об этом говорит.
    ...
    # Платёж 20-го числа — аванс за ИЮНЬ (во второй половине месяца платят вперёд), значит
    # УПД по нему ждут до 10 июля. Порог 16-го подтверждён владельцем на его данных:
    # Директ и Синапсис платят 29-го, ДоксИнБокс 23-го — и это всегда следующий месяц.
    assert waiting["expected_by"] == "2026-07-10"
