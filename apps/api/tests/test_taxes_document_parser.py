...
# ── период по имени файла и заголовку ────────────────────────────────────────


@pytest.mark.parametrize(
    "text",
    [
        "Г. ВОЛГОДОНСК, ЦВЕТОЧНЫЙ Б-Р, 33",  # город плательщика в форме ПД
        "УЛ МИРА, Д 21 КВ 3",  # «21 КВ» — не «1 кв»
        "выгодно",
        "загодя",
        "негодный",
    ],
)
def test_period_hint_ignores_words_that_merely_contain_a_marker(text: str) -> None:
    """Слово, внутри которого случайно лежит маркер, периода не задаёт.

    «ВОЛГОДОНСК» содержит «год»: месячный взнос на травматизм объявлялся ГОДОВЫМ,
    обязательство уезжало в слот for_period='year' и затирало там предыдущий месяц.
    """
    assert document_parser._period_hint("", text) is None


@pytest.mark.parametrize(
    ("filename", "header", "expected"),
    [
        # маркер в имени файла — как называет бухгалтер
        ("УСН 1 кв до 28.04.docx", "", "q1"),
        ("УСН 1 квартал.docx", "", "q1"),
        ("СВ ИП 2 кв.docx", "", "h1"),
        ("УСН полугодие.docx", "", "h1"),
        ("Отчёт 9 мес.docx", "", "9m"),
        ("УСН 3 кв.docx", "", "9m"),
        ("УСН за год.docx", "", "year"),
        ("УСН 4 кв.docx", "", "year"),
        ("ЕНП 07.2026.docx", "", "2026-07"),
        # ПОДЧЁРКИВАНИЕ: словесный символ, границы \b после него нет. Бухгалтер так
        # и называет файлы («ЕНП_до 28.08.docx»), поэтому маркер обязан находиться.
        ("ЕНП_год.docx", "", "year"),
        ("УСН_1 кв.docx", "", "q1"),
        ("СВ ИП_2 кв.docx", "", "h1"),
        ("УСН_полугодие.docx", "", "h1"),
        ("отчет_9 мес.docx", "", "9m"),
        # маркер слитно с соседом
        ("усн за 2026год.docx", "", "year"),
        ("УСН за2кв до 28.07.docx", "", "h1"),
        ("УСН1кв.docx", "", "q1"),
        # маркер в ЗАГОЛОВКЕ документа — на проде он живёт именно там
        ("", "УСН 2 КВ  до 28.07", "h1"),
        ("", "ЕНП   1% СВ за 2 кв-оплатить до 25.09", "h1"),
    ],
)
def test_period_hint_reads_real_periods(filename: str, header: str, expected: str) -> None:
    """Живые формы читаются как раньше — правка отсекает подделки, а не маркеры."""
    assert document_parser._period_hint(filename, header) == expected


def test_payment_order_reads_period_from_header_not_from_body() -> None:
    """В период смотрят ТОЛЬКО имя файла и первая строка — не всё тело документа.

    На этом держится безопасность признанной неоднозначности: адрес «д 1 кв 5» неотличим
    от настоящего «1 кв», и единственное, что не даёт ему стать периодом, — что адрес до
    этой функции не доходит. Тест закрепляет границу: начни кто-нибудь передавать сюда
    весь текст — упадёт здесь, а не в реестре обязательств.
    """
    assert document_parser._period_hint("", "Г. ВОЛГОДОНСК, УЛ МИРА, Д 1 КВ 5") == "q1"

    doc = _po("usn_h1_478376.docx", "УСН 2 кв до 28.07.docx")
    body = document_parser._docx_text((FIXTURES / "usn_h1_478376.docx").read_bytes())
    lines = [ln.strip() for ln in body.split("\n") if ln.strip()]
    assert len(lines) > 1, "фикстура должна быть многострочной, иначе тест ничего не ловит"
