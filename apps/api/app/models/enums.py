...
counterparty_invoice_source_enum = Enum(
    ...
    "self_billed",
    # utility = возмещение коммунальных расходов арендодателю (вода/газ/свет). Счёт ресурсника
    # выставлен на него, а не на нас: сумму подтверждает его расчёт, первички на ИП нет —
    # значит расход управленческий, но не налоговый. Заводится проведением платёжки в модуле
    # «Коммунальные услуги», а не по расписанию: сумма каждый месяц своя.
    "utility",
...
SELF_ACCRUED_INVOICE_SOURCES = frozenset({"lease", "self_billed", UTILITY_INVOICE_SOURCE})
