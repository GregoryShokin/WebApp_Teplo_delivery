...
class DdsArticle(Base):
    ...
    # Движение по статье обязано называть СОБСТВЕННИКА (взнос в бизнес, возврат ему, дивиденды).
    # Собственников двое, у каждого свои расчёты с бизнесом — без имени в проводке «поступление
    # от собственников» это общий котёл, из которого нельзя вынуть, кто сколько внёс и кому
    # сколько должны. Собственник живёт обычным контрагентом с ролью ``owner``: так у каждого
    # появляются собственные ДЗ/КЗ, а проводке не нужна ещё одна колонка.
    owner_required: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=False, server_default=text("false")
    )
