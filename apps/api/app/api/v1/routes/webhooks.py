...
async def _ingest_tbank_account_operation(
    ...
    Realtime-выписка приходит на тот же URL, что и статус черновика. Сначала безусловно пишем
    операцию в выписку (баланс банка + пикер карт-оплат Кассы), затем отдельной транзакцией
    пробуем закрыть черновик по подтверждённой Transaction, сумме и назначению. Анти-дубль с
    оплатой черновика — общий prebooked-механизм классификатора. Холд (``authorization``) в
    баланс не пускаем. Дедуп по ``operationId`` + стабильному ключу выписки в
    ``ingest_operations`` делает повторную доставку идемпотентной.
    """
    ...
    # Near-realtime доводка ОПЛАТЫ черновика: новая исходящая Transaction может быть исполнением
    # нашего банковского черновика — матчим её по сумме/назначению/счёту/documentNumber, не ждём
    # отстающий статус документа. Только для НОВОЙ операции (inserted>0), чтобы переотдача
