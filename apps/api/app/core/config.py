...
class Settings(BaseSettings):
    ...
    # модели прошлого поколения — 1568 px, и на этой разнице цифры перестают быть различимыми.
    # Цена вопроса при трёх ресурсах — несколько долларов в год, поэтому экономить тут нечего.
    utility_ocr_model: str = "claude-opus-5"

    # --- Телеграм-бот приёмки документов -------------------------------------------------
    # Куда владелец пересылает фотографии от арендодателей. Бот НИЧЕГО не решает сам: он
    # доставляет файл в ту же дверь, что и кнопка «Загрузить документ» на «Странице на оплату».
    #
    # Белого списка чатов НЕТ — решение владельца 02.08.2026. Принимаем от любого, кто написал
    # боту; взамен каждая строка приёмки помнит отправителя (имя и chat id уходят в поле
    # «от кого» на «Странице на оплату»), поэтому вопрос «чей это документ» отвечается всегда.
    telegram_intake_enabled: bool = True
    telegram_intake_bot_token: str | None = None
    # Насколько старое сообщение бот ещё берёт в работу. Телеграм хранит неподтверждённые
    # обновления сутки и отдаёт их все разом при первом запуске — после деплоя это значило бы
    # пачку вчерашних документов, часть из которых уже проведена руками. Час покрывает
    # перезапуск и короткий простой, но не разбор завалов. Ноль отключает отсечку.
    telegram_intake_max_message_age_minutes: int = 60
    # Долгий опрос (long polling): соединение висит до появления сообщения. Так документ
    # доезжает за секунды, а не за интервал, и вебхук с публичным адресом не нужен.
    telegram_intake_poll_timeout_seconds: int = 25
    telegram_intake_poll_interval_seconds: int = 30
