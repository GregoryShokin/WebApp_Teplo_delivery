...
# Решение владельца 02.08.2026 взамен прежнего «конец периода»: УПД за август 31 августа не
# выставит никто — он приходит в первых числах сентября. Прежний срок делал строку красной
# 1-го числа заведомо зря, а ложное красное обесценивает настоящую просрочку.
DEFAULT_CLOSING_DOC_DAY = 10
...
def expected_by(period_end: date, expected_day: int | None) -> date:
    ...
    оказания, поэтому за август ждать его в августе бессмысленно. И не в последний день
    периода: за август УПД выставят в сентябре, а не 31.08.

    Заданное в карточке число — день, в который контрагент обычно присылает документ: в этот
    день ещё ждём, краснеет со следующего. Иначе «присылает 5-го» и «просрочил 5-го» означали
    бы одно и то же, и поле теряло бы смысл.
    """
    if expected_day is None:
        expected_day = DEFAULT_CLOSING_DOC_DAY
...
async def _payment_rows(
    ...
    # Входящие остатки — деньги ушли до внедрения системы, и своей ДДС-проводки у них нет.
    # Только они и есть самостоятельный денежный факт: у всех прочих предоплат без проводки
    # деньги уже учтены чем-то другим (оплатой счёта, оплатой накладной, откатом расхода), и
    # вторая строка в хронологии задваивала бы остаток. Раньше отсекались только оплаченные
    # счета (``prepaid_bill``), и излишек оплаты накладной ООО «АЛЬЯНС ЮГ» на 23 730 ₽ уводил
    # сверку от плитки «Остатки» ровно на эту сумму.
