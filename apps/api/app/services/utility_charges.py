...
async def build_utility_documents(
    ...
    *,
    ...
    await supplier_prepayments.apply_closing_document(
        ...
        # Не назвал — не гасится арендой и деньгами другого потока, но это правило живёт в
        # ядре зачёта (``supplier_prepayments._other_stream_prepayment_ids``), а не здесь: акт
        # гасится и другими дверями — активацией 1-го числа, обратным порядком при оплате
        # чужого счёта, — и белый список одной двери их не закрывал.
