...
* аренда (``lease_id``) необязательна для обычной ``location_required`` статьи (коммуналка,
  охрана): помещение есть, а договора нет — платят свободному контрагенту;
* но для статьи-аренды (``lease_bound``) договор ОБЯЗАТЕЛЕН: получатель этой статьи — только
  зарегистрированный арендодатель помещения, свободного контрагента тут быть не может;
    ...
    *,
    ...
    if lease_id is None:
        if article.lease_bound:
            raise LocationAnalyticsError(
                f"Для статьи «{article.name}» выберите арендодателя (договор аренды помещения)"
            )
