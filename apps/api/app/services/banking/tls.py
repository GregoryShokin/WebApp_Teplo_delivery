...
Тот же корень лежит в ``deploy/certs/russian_trusted_root_ca.pem`` — из него собирается
бандл Сбера. Здесь своя копия: контекст сборки образа — ``apps/api``, ``deploy/`` в него не
попадает. Обе копии закреплены одним отпечатком SHA-256 (см. тест ``test_bank_tbank_tls``).
"""
