...
# Сокращения месяцев из счетов студии Синапсис («Авг. 2026») и Яндекс.Директа. Держим их
# ОТДЕЛЬНО от полных названий: полное имя однозначно, а «мар»/«мая» коротки настолько, что в
# общем словаре ловили бы куски чужих слов. Здесь они применяются только своим шаблоном,
# который требует год рядом и запрещает день перед месяцем.
_RU_MONTH_ABBR = {
    "янв": 1,
    "фев": 2,
    "мар": 3,
...
_RU_MONTH_ABBR_PATTERN = "|".join(sorted(_RU_MONTH_ABBR, key=len, reverse=True))
...
# Полные формы идут первыми: в бланке УПД поставщик пишется прописью («Общество с ограниченной
# ответственностью "ЭкоЦентр"»), аббревиатуры там нет вовсе — без них имя продавца не находилось,
# уверенность падала ниже порога, и документ уходил в ручной разбор.
_ORG_RE = re.compile(
    r"(?:Общество\s+с\s+ограниченной\s+ответственностью|Акционерное\s+общество"
    r"|Публичное\s+акционерное\s+общество|ООО|ОАО|ЗАО|ПАО|НАО|АО|ИП)"
    r"\s+[«\"’']?[^»\"’'\n;:|/]{2,80}",
    re.IGNORECASE,
)
...
def _extract_service_periods(text: str) -> list[tuple[date, date, str, float]]:
    ...
    # Месяц БЕЗ маркера «за»/«период» — просто дописан к наименованию услуги:
    #
    #     «…абонентское обслуживание Авг. 2026»            (Синапсис, Яндекс.Директ)
    #     «…дом № 4В, пом 1 (Июль 2026 г.) 1 мес»          (ЧОО «Охрана Юг», СпецАвто Юг)
    #
    # Формы две — сокращённая и полная в скобках, — и обе на проде оставались нераспознанными:
    # платежи этим четверым вставали в очередь и ждали документа, которого либо не будет
    # вовсе, либо он и так приходит вместе со счётом.
    #
    # ДЕНЬ ПЕРЕД МЕСЯЦЕМ ЗАПРЕЩЁН, и это главная защита шаблона. В тех же счетах рядом стоит
    # «к договору SUP-484108 от 12 Сен. 2025г.» — дата договора, а не период услуги. Отличает
    # их ровно одно: у периода дня нет. Без этого запрета счёт Синапсиса получил бы период
    # «сентябрь 2025», то есть расход уехал бы на год назад.
    bare_month = re.compile(
        rf"(?<![\d.])[\s(]({_RU_MONTH_PATTERN}|{_RU_MONTH_ABBR_PATTERN})\.?\s*(\d{{4}})\b",
        re.IGNORECASE,
    )
    for match in bare_month.finditer(text):
        month_name, year_raw = match.groups()
        ...
        # Уверенность ниже, чем у явного «за период: …»: формулировка слабее, и при споре
        # с явной побеждает явная.
        found.append((start, end, "document_month_abbr", 0.88))
...
def _pick_inn(text: str) -> str | None:
    """ИНН контрагента из текста документа; наши собственные ИНН пропускаем.

    Форма ``ИНН/КПП продавца: 7839494297/781101001`` — это бланк УПД и счёта-фактуры, и
    поддержать её обязательно: без ИНН документ не находит контрагента и падает в ручной разбор.
    Так вышло с УПД «Назад в будущее» и «ЭкоЦентра» — ИНН в тексте есть, но между словом «ИНН» и
    цифрами стоит «/КПП продавца:». Какой из двух ИНН бланка взять, решает фильтр ``OWN_INNS``:
    наш — это покупатель, значит нужен другой.
    """
    seen: list[str] = []
    pattern = (
        r"ИНН(?:\s*/\s*КПП)?"
        r"(?:\s*(?:продавца|поставщика|покупателя|получателя|плательщика))?"
        r"[\s:№]*(\d{12}|\d{10})"
    )
