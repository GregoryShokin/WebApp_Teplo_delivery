...
def _extract_service_periods(text: str) -> list[tuple[date, date, str, float]]:
    ...
    # «…абонентское обслуживание Авг. 2026», «Размещение рекламных материалов — Авг. 2026».
    #
    # Здесь нет ни слова «за», ни слова «период»: месяц просто дописан в конец наименования
    # услуги. Так выставляют Синапсис и Яндекс.Директ, и на проде их счета оставались без
    # периода — платёж вставал в очередь и ждал документа, которого эти контрагенты не шлют.
    #
    ...
    abbr_phrase = re.compile(
        rf"(?<![\d.])\s({_RU_MONTH_ABBR_PATTERN})\.?\s*(\d{{4}})\b",
        re.IGNORECASE,
    )
    for match in abbr_phrase.finditer(text):
        month_name, year_raw = match.groups()
        month = _RU_MONTH_ABBR.get(month_name.casefold())
        if month is None:
            continue
        year = int(year_raw)
