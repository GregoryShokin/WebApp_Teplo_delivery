...
async def create_warehouse_invoice(
    ...
    *,
    ...
    # Общий хук активирует закрывающий документ по дате. Для warehouse он не гасит аванс
    # автоматически: конкретную товарную поставку оператор связывает с предоплатой вручную.
    # Хук строго ПОСЛЕ финализации amount (финансовый контур считает зачёт от остатка).
