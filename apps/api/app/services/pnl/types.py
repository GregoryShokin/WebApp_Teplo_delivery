...
class Component:
    ...
    # Исключено из расхода, но ждать документа не от кого: платежа нет в ДЗ/КЗ вовсе.
...
class Reconciliation:
    ...
    Задвоение шумит — расходятся итоги. Потеря молчит. Поэтому потерю ловим тождеством:
    отток месяца обязан без остатка разложиться по вердиктам, а ``unmapped`` — быть нулём.
    """
    ...
    balanced: bool = True
    # Расхождение левой и правой частей. Ненулевое означает ошибку в самом проекторе.
