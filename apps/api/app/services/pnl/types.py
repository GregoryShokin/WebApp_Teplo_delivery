...
class Verdict(StrEnum):
    ...
    # Месяц расхода раньше начала учёта: расход относится к периоду, которого в отчётах нет
    # вовсе. Деньги месяца учтены, расход не появится нигде — и это ЗАКОННЫЙ исход, а не
    # потеря. Отдельный вердикт от ``INCLUDED_OTHER_MONTH`` именно потому, что там расход
    # ждёт своего месяца, а здесь ждать нечего: доплата за июнь 2026 по акту электричества
    # закрывает период до 01.07.2026, и её место — во входящих остатках, а не в ОПиУ.
    EXCLUDED_BEFORE_ACCOUNTING_START = "excluded_before_accounting_start"
...
class Component:
    ...
    # Состояние этого ожидания (``waiting.STATE_*``): период идёт, документ получен, ждём до
    # срока, просрочено. Тревога — только просрочка; остальное — пометка на строке.
