...
БЕРЁТСЯ НЕДОСТАЧА ВСЕГО ЗА ВЫЧЕТОМ ИЗЛИШКОВ. У ревизии три разных числа, и перепутать их
легко:
...
* ``audit.total_shortage_amount`` — «Учитываемая в штрафе»: только активные позиции,
  15 044,18 ₽ по той же ревизии. Это база расчёта штрафа, а не потеря бизнеса, и в ОПиУ она
  не идёт — решение владельца 04.08.2026;
* ``sum(item.amount)`` — знаковый итог: недостача отрицательна, излишек положителен. За ту же
  ревизию −16 847,52 ₽.

ИЗЛИШКИ УЧАСТВУЮТ В РАСЧЁТЕ — решение владельца 05.08.2026: «они тоже должны влиять на расчёт
прибыли». Раньше строка брала одну недостачу, а излишки показывались справочно: считалось, что
это пересорт и ошибки учёта, и зачитывать их значит прятать настоящую потерю. Владелец решил
иначе, и это последовательно: если излишек — тоже ошибка учёта, то он ровно так же искажает
прибыль, только в другую сторону, и оставлять в отчёте половину расхождения нельзя.

ФОРМУЛА ПРЯМАЯ: сумма недостач минус сумма излишков, без всякого зачёта пересорта — владелец
сказал это отдельно и явно. Поэтому обе величины считаются СВОИМИ суммами, а не через знаковый
``item.amount``. Разница не косметическая: ``amount`` приходит из iiko как есть и совпадает с
``-shortage_amount`` только пока iiko ничего не зачёл внутри позиции; опираться на него значит
молча пустить в отчёт чужую логику пересорта. За июль 2026 итог 45 841,38 − 19 347,65 =
26 493,73 ₽ вместо прежних 45 841,38 ₽.

У СОСЕДНИХ строк (инвентаризация упаковки, коробок для пиццы и напитков) источник другой:
...
class InventoryMonth:
    ...
    #: Величина строки ОПиУ: недостача минус излишки. Может стать отрицательной — месяц, где
    #: излишков нашли больше, чем недостач, уменьшает расход, а не обнуляется в ноль.
    product_result: Decimal | None = None
    #: Составляющие итога, каждая своей суммой. Нужны расшифровке и объяснению в отчёте.
    shortage_amount: Decimal = Decimal("0.00")
...
async def build_inventory_month(
    ...
    # Излишек — это положительный ``amount`` позиции; недостача живёт своим полем. Считаем
    # ДВЕ суммы, а не один знаковый итог: связывать их через ``amount`` значило бы протащить
    # в отчёт зачёт пересорта, сделанный на стороне iiko, а владелец просил простую разность.
    shortage_total, surplus_total = (
        await session.execute(
            select(
                func.coalesce(func.sum(InventoryAuditItem.shortage_amount), 0),
                func.coalesce(
                    func.sum(
                        func.greatest(InventoryAuditItem.amount, Decimal("0.00")),
                    ),
                    0,
                ),
            ).where(*conditions)
    ...
    shortage = Decimal(shortage_total or 0)
    surplus = Decimal(surplus_total or 0)
    return InventoryMonth(
        product_result=shortage - surplus,
