...
async def load_packaging_guids(session: AsyncSession) -> set[str]:
    """Товары, чьё расхождение уходит в СВОИ строки барной ревизии.

    Берём по ``line_code``, а не по статусу: эти товары размечены как складские (``stocked``,
    это нужно балансу) и при этом имеют строку ОПиУ. Именно пара «складской + со строкой» и
