...
становится только то, что требует действия: просрочка и документ, который не вступил в свой
день; остальные — пометка на строке.
"""
...
#: Состояния ожидания — от «делать нечего» к «пора действовать». Тревога — два последних.
#:
...
#: Срок вышел, документа нет. Действие — написать контрагенту.
STATE_OVERDUE = "overdue"
#: Документ или начисление лежит в системе, его день прошёл, а он так и не вступил: ночная
#: проводка не отработала или месяц закрыт (``recognize_due_expenses`` в закрытый месяц не
#: пишет). Само уже не встанет. Действие — не контрагенту, а в систему: документ у нас есть.
STATE_STALLED = "stalled"
#: Состояния, которые требуют действия, — тревога. Остальные законны и стоят пометкой.
ALARM_STATES = frozenset({STATE_OVERDUE, STATE_STALLED})
...
class WaitingItem:
    ...
    #: Сколько дней тревоге: от срока документа (``overdue``) или от дня, когда должен был
    #: вступить документ (``stalled``).
    overdue_days: int = 0
    #: Для ``document_pending`` и ``stalled`` — чем подтвердится: ``BASIS_*``.
    basis: str | None = None
...
def waiting_state(
    *,
    ...
    НО ДОКУМЕНТ ОТВЕЧАЕТ ТОЛЬКО ДО СВОЕЙ ДАТЫ. Признание и активация идут ночью в сам день
    ``incoming_on`` (00:05 и 00:10 по Москве), поэтому весь этот день документ ещё законно
    ждёт. Со следующего дня «вступит 01.10» — уже неправда: система не сделала обещанного, и
    сама не сделает. До 25.09.2026 такое ожидание висело «вступит 01.10» вечно: реплей копии
    прода на 15.11 держал так 137 685,59 ₽ в пяти строках (аренда, склад, АЙКО, ЧОО,
    коммуналка), когда всё остальное за сентябрь давно стало просрочкой.

    Зависший документ НЕ проваливается в проверку срока, хотя так было бы проще. Там он
    получил бы чужие слова — «ждём документ», «запросите у контрагента» — о бумаге, которая
    уже у нас, а об аренде и вовсе о бумаге, которую арендодатель не выставляет. И тревога
    опоздала бы до срока документа (10-е, у коммуналки 20-е): полмесяца, в которые отчёт
    выглядит спокойным, а деньги так и не стали расходом. Поэтому у зависшего своё состояние,
    тревога — сразу, а дни считаются от дня, когда он должен был вступить.
    """
    if incoming_on is not None:
        if incoming_on >= today:
            return STATE_DOCUMENT_PENDING, incoming_on, 0
        return STATE_STALLED, incoming_on, (today - incoming_on).days
    if period_end >= today:
...
def state_label(item: WaitingItem) -> str:
    ...
    if item.state == STATE_STALLED:
        if item.basis == BASIS_DOCUMENT:
            return f"документ получен, должен был вступить {on} — не вступил"
        if item.basis == BASIS_AGREEMENT:
            return f"по договору должно было начислиться {on} — не начислено"
        return f"расход должен был начислиться {on} — не начислен"
...
def _match_incoming(
    ...
    *,
    ...
    СУММУ И ДОЛЮ МЕСЯЦА НЕ СВЕРЯЕМ — намеренно, решение 25.09.2026. Сужение по сумме
    покраснело бы там, где всё в порядке: акт на два оплаченных счёта, копейки НДС, частично
    зачтённый аванс, у которого остаток меньше документа. Сужение по доле месяца врёт на
    документе без периода: его период — догадка по дате, а гасит он аванс целиком. Цена
    широкого сопоставления при этом ограничена: в свой день документ гасит то, что гасит, и
    непогашенный остаток теряет пару и сразу уходит в календарь (срок документа — не раньше
    10-го, а документ с периодом вступает 1-го). Не вступивший документ не прячет ничего — он
    сам тревога. На проде 25.09 все девять отложенных документов — ровно сентябрьские и один к
    одному по сумме с авансами: сужение не изменило бы ни одной строки.
    """
    matched = [
        candidate
    ...
    if not matched:
        return None
    # Что должно было вступить и не вступило — важнее того, что вступит позже: ожидание
    # закрыто, когда вступит последнее из подходящих, но зависшее само не вступит никогда, и
    # более поздний документ не вправе его заслонять.
    missed = [candidate for candidate in matched if candidate.on < today]
    if missed:
        on = min(candidate.on for candidate in missed)
        named = [candidate for candidate in missed if candidate.on == on]
    else:
        on = max(candidate.on for candidate in matched)
        named = matched
    basis = (
        BASIS_DOCUMENT
        if any(candidate.basis == BASIS_DOCUMENT for candidate in named)
        else named[0].basis
    )
    return on, basis
