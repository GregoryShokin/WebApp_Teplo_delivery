            ...
            candidates = (
                await session.scalars(
                    select(SupplierInvoice).where(
                        ...
                        # Накладная должна была РЕАЛЬНО уходить в iiko. Без этого условия в пул
                        # кандидатов попадала любая наша накладная с тем же номером, а серии
                        # номеров у складских, кассовых и финансовых документов общие
                        # (515243, 515244, 515111…). Совпадение номера само по себе не значит
                        # ничего — «ровно один кандидат» лишь маскировало проблему: при
                        # единственном похожем номере чужой документ получал чужой external_id
                        # и переставал синхронизироваться навсегда.
                        #
                        # 'failed' здесь обязателен и не является послаблением: ради него
                        # fallback и написан. Пуш, оборвавшийся на ответе iiko (таймаут),
                        # оставляет статус 'failed' с пустым external_id, а документ в iiko
                        # при этом создан. Отсекаются 'not_pushed' и 'skipped' — эти в iiko
                        # не уходили вовсе, и совпадение номера у них случайно.
                        SupplierInvoice.iiko_push_status.in_(("pushed", "failed")),
                    )
                )
            ).all()
            if len(candidates) == 1:
