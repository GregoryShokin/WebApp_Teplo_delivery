...
различаются не названием, а тем, в какой раздел ДДС попадут деньги: капитальная —
``investing``, текущая — ``operating``. Поэтому сумма обязана сходиться с выбранной статьёй,
а реакция на расхождение у сторон разная:

* дешёвый расход по КАПИТАЛЬНОЙ статье — ОТКАЗ. Стоимость объекта он не увеличит, значит
  деньги ушли бы в инвестиции, а затрата осталась бы в расходах периода. Исправление
  бесплатное: рядом лежит операционная статья;
* дорогой расход по ТЕКУЩЕЙ статье — ПРЕДУПРЕЖДЕНИЕ. Дорогой текущий ремонт законен
...
НАЗВАНИЯ СТАТЕЙ В ЭТИХ СООБЩЕНИЯХ БЕРУТСЯ ИЗ БАЗЫ, А НЕ ИЗ КОДА (замечание владельца
2026-07-31). Каталог статей владелец правит из интерфейса, и оба текста — «проведите по такой-
то статье» — переживают переименование только если имя читается на месте. Захардкоженное имя
после ближайшего же переименования отправит человека искать статью, которой больше нет: ошибка
покажется ему враньём системы, а не своей опечаткой в выборе статьи.
...
# Запасные подписи для случая, когда парной статьи в каталоге нет вовсе. Сообщение с ними
# всё равно осмысленно («заведите такую статью»), а падать из-за отсутствия справочника
# проверка не имеет права.
_KIND_FALLBACK_NAMES = {"repair": "Капитальный ремонт ОС", "maintenance": "Ремонт оборудования"}


async def _article_name_for_kind(session: AsyncSession, kind: str) -> str:
    """Как СЕЙЧАС называется статья с таким признаком — для подсказки «проведите по …».

    Читаем из каталога, а не из константы: имена статей владелец правит из интерфейса, и
    подсказка, ссылающаяся на устаревшее имя, хуже отсутствия подсказки — она посылает
    человека искать несуществующее.
    """
    name = await session.scalar(
        select(DdsArticle.name)
        .where(DdsArticle.asset_link_kind == kind, DdsArticle.is_active.is_(True))
        .order_by(DdsArticle.name)
        .limit(1)
    )
    return name or _KIND_FALLBACK_NAMES[kind]
...
async def _repair_context(
    ...
    ОТКАЗ, А НЕ ПРЕДУПРЕЖДЕНИЕ (решение владельца 2026-07-30). Капитальная статья в каталоге —
    ...
    операционных. Исправить это человеку ничего не стоит — рядом лежит текущая статья,
    ...
    if verdict == "repair":
        ...
        pair = await _article_name_for_kind(session, "maintenance")
        raise AssetLinkError(
            f"Расход {Decimal(str(amount)):.2f} ₽ на объект «{asset.name}» — {reason}. "
            f"Стоимость объекта он не увеличит, поэтому проведите его как «{pair}»: иначе "
            f"деньги уйдут в инвестиции, а затрата останется в расходах периода"
        )
...
async def _maintenance_context(
    ...
    симметрично: раз расход тянет на капитальный ремонт, пусть идёт по капитальной статье. Но
    ...
    reason: str | None = None
    if verdict == "upgrade":
        pair = await _article_name_for_kind(session, "repair")
        reason = (
            ...
            f"стал дороже, проведите расход по статье «{pair}»"
        )
    return AssetContext(
