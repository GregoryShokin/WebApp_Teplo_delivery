...
отправителю (налоговый агент), разобрать, положить в staging со статусом. Повторный проход
того же письма не плодит строк; одинаковые байты в письмах разных месяцев обрабатываются
отдельно, так как месяц травматизма может определяться датой письма.
...
async def ingest_tax_documents(
    ...
    *,
    ...
    Идемпотентно: вложение того же письма с тем же SHA-256 пропускается. ``fetch`` и ``accounts``
        ...
        for att in attachments:
            ...
            exists = await session.scalar(
                select(TaxDocumentIntake.id).where(
                    TaxDocumentIntake.mailbox == att.mailbox,
                    TaxDocumentIntake.message_uid == att.message_uid,
