...
def _period_hint(filename: str, header: str) -> str | None:
    r"""Отчётный период из имени файла и заголовка документа.

    Маркер отсекается ровно тем, что может его подделать, и ничем больше:

    * буквенному («год», «полугод») мешает БУКВА слева. Без этого «год» находился внутри
      «ВОЛГОДОНСК» — города плательщика в форме ПД, — и месячный взнос на травматизм
      объявлялся ГОДОВЫМ: обязательство уезжало в слот ``for_period='year'`` и затирало
      там предыдущий месяц;
    * цифровому («1 кв», «9 мес») мешает ЦИФРА слева. Без этого «Д 21 КВ 3» в адресе
      читалось как «1 кв», то есть I квартал.

    Почему не ``\b``: подчёркивание — словесный символ, границы после него нет, и
    «ЕНП_год.docx» с «УСН_1 кв.docx» теряли период молча. А потерянный период хуже
    неверного: платёжка без периода проходит как уверенно распознанная, в сверке
    (``reconcile._documented_amount``) отдаёт свою сумму СРАЗУ ВО ВСЕ периоды, а при
    продвижении садится в общий NULL-слот, где затирает предыдущую.

    Что не различается и различено быть не может: адрес «д 1 кв 5» выглядит как «1 кв»
    ровно так же, как настоящее «УСН 1 кв.docx» — в обоих номер стоит перед «кв». Сегодня
    это недостижимо: сюда приходят только имя файла и ПЕРВАЯ строка документа
    (``parse_payment_order``), адреса там нет. Понадобится скармливать весь текст —
    разделять придётся не регуляркой, а тем, откуда взята строка.
    """
    text = f"{filename} {header}".lower()
    if re.search(r"(?<!\d)1\s*кв", text):
        return "q1"
    if re.search(r"(?<!\d)2\s*кв|(?<![а-яёa-z])полугод", text):
        return "h1"
    if re.search(r"(?<!\d)3\s*кв|(?<!\d)9\s*мес", text):
        return "9m"
    if re.search(r"(?<![а-яёa-z])год|(?<!\d)4\s*кв", text):
        return "year"
