...
class YearConfig:
    ...
    # Детские вычеты НДФЛ (ст. 218 НК, размеры с 2025): на 1-го, на 2-го, на 3-го и
    # каждого следующего; СУММИРУЮТСЯ по детям, единственному родителю — вдвое.
    # Вычет действует, пока доход нарастающим итогом не превысил лимит.
    child_deduction_first: Decimal = Decimal("1400.00")
    child_deduction_second: Decimal = Decimal("2800.00")
    child_deduction_next: Decimal = Decimal("6000.00")
    ndfl_deduction_income_limit: Decimal = Decimal("450000.00")
...
YEAR_CONFIGS: dict[int, YearConfig] = {
    ...
    # 2027 — ПРЕДВАРИТЕЛЬНО параметрами 2026 (МРОТ/фикс/пороги обновить, когда выйдут законы).
    # Без конфига следующего года контур 1 января МОЛЧА гаснет: year_config бросает
    # TaxComputationError, витрина активных платежей глотает его и показывает пусто
    # (находка аудита 27.07.2026). Предварительные цифры честнее пустоты — сверка с
    # платёжками бухгалтера расхождение подсветит.
    2027: YearConfig(year=2027),
}
