 ...
 *
 *  Порядок: правки оператора → (для получателя карточка, затем счёт) → остальные поля счёта
 *  → карточка → пусто. Наименование банка иногда ошибочно распознаётся как получатель;
 *  для уже найденного контрагента его карточка надёжнее. Ничего не применяется молча:
...
export function buildPrefill(input: PrefillInput): {
  ...
  REQUISITE_KEYS.forEach((key) => {
    ...
    // Название получателя — идентичность уже выбранного контрагента, а не часть банковского
    // блока. OCR иногда принимает строку «Банк получателя» за название организации; карточка
    // контрагента в этом случае надёжнее и не должна быть затёрта распознанным банком.
    if (key === "recipientName" && card.recipientName) {
      values[key] = card.recipientName;
      sources[key] = "card";
      return;
    }
    if (recognized[key]) {
