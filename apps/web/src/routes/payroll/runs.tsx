...
export function PayrollRunsRoute({
  ...
  const visibleTabs = [
    // «Расчёты» и «Администрация» не назывались тем, чем являются: первая вкладка — про
    // производственный персонал (недельные ведомости), вторая — про административный
    // (полумесячные). Пара «производство / администрация» читается сразу и в одном ряду
    // с остальными вкладками не теряется (решение владельца 03.08).
    permissions.canOpenSection("payroll.runs")
      ? { value: "runs", label: "Зарплата производства" }
      : null,
    permissions.canOpenSection("payroll.runs")
      ? { value: "admin", label: "Зарплата администрации" }
      : null,
  ...
  // Полумесячные ведомости во вкладку «Зарплата производства» не пускаем: их место —
  // «Зарплата администрации». Бэкенд уже отдаёт только недельные, фильтр здесь — страховка
  ...
  return (
    <div className="space-y-5">
      {/* Заголовок повторяет пункт меню, а описание больше не обещает «еженедельные
          расчёты»: страница давно шире одной вкладки — у администрации период
          полумесячный, а депозиты, фонд и авансы к периоду ведомости не привязаны вовсе. */}
      <PageHeader
        title="Зарплата"
        description="Производство — недельные ведомости вторник-понедельник, выплаты по вторникам; администрация — полумесячные."
      />
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange} className="space-y-5">
        ...
        {invalidPath ? (
          <EmptyState
            ...
            // Перечислять вкладки поимённо здесь — гарантия отстать: список уже не знал про
            // авансы, а теперь и про депозиты. Ряд вкладок стоит прямо над этим текстом.
            description="Такой вкладки в зарплате нет — выберите нужную в списке выше."
          />
        ) : (
          <>
            {permissions.canOpenSection("payroll.runs") ? (
