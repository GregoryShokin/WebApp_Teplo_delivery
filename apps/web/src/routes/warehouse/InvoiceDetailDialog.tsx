      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        ...
        {detailQuery.isLoading || !detail ? (
          ...
          <div className="grid gap-4">
            ...
            {/* Период оказания услуги — для любого услугового контрагента, а не только для
                того, кому период ОБЯЗАТЕЛЕН. Пока блок висел на `service_period_required`,
                выключенном у всех карточек, вписать период было негде — а ОПиУ при этом
                просил «заполните период в ДЗ/КЗ». Для накладной не из почты это единственная
                точка ввода; пока период не заполнен, счёт не уйдёт в банк. */}
            {invoiceId && (detail.service_period_editable ?? detail.service_period_required) ? (
