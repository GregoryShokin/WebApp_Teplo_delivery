...
Документ закрывает связь snapshot-сервиса `research/raw/courier_service/` с уже описанной книгой `График курьеров и показатели курьеров` и фиксирует, что именно переносить в `apps/api`.
