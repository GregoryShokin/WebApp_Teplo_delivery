...
Статус: default-решение для первого скелета монорепо. PostgreSQL уже зафиксирован архитектурным решением 30; остальные пункты выбраны как минимально сложный self-hosted стек под 5-10 пользователей, read-heavy нагрузку и постепенный перенос Python-интеграций из `scripts/`.
...
- Существующие интеграции уже написаны как Python-скрипты в `scripts/`, поэтому первый backend должен переиспользовать этот язык и рантайм.
...
scripts/   существующие Python-интеграции и агенты; переносить постепенно
...
- `scripts/` пока остаются рабочими интеграциями. Новые адаптеры в `apps/api/app/integrations` должны вызывать или переиспользовать их логику без копирования секретов в код.
- `apps/web` не хранит бизнес-логику расчетов; он показывает состояния, отчеты, формы ручного подтверждения и audit evidence.
- `data/private/` остается private-хранилищем raw artifacts; в БД и processed-слое хранятся ссылки, hashes, статусы качества и нормализованные поля.
...
- Перенос всех `scripts/` внутрь backend: миграция должна идти по одному источнику, с audit trail и fixtures.
