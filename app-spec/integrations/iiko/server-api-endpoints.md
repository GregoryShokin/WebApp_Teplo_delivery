...
| ФОТ / явки | iiko + Google Sheets | `/employees/attendance`, `Расчет зарплат NEW` | Еженедельно | Факт явки брать из iiko; правила начислений, депозиты, фонд и выплаты остаются по payroll-логике, см. [22-iiko-employees-api.md](/app-spec/integrations/iiko/employees-api.md) и [payroll engine spec](/app-spec/modules/staff/payroll/00-engine.md) |
...
## Склад: СОЗДАНИЕ акта списания (iiko Cloud)

Read-only `GET /resto/api/v2/documents/writeoff` выше — только чтение. Создавать акты умеет
**iiko Cloud**, ресурс `writeoff_document`. Проверено на живом API 05.08.2026 (проба создана
и сразу отменена, акт `0458`).

```text
POST https://api-ru.iiko.services/api/inventory/v1/writeoff_document/create
POST .../writeoff_document/{get,list,post,unpost,cancel,update}
```

Полный цикл тот же, что у накладных (`incoming_invoice`), и обслуживается тем же
`iiko_cloud_call`: Bearer-токен marketplace-приложения, тело JSON.

### Тело `create`

| Поле | Обяз. | Значение |
| --- | --- | --- |
| `organizationId` | да | `5c7e51f9-93c6-450c-9299-440ac1c889e8` |
| `date` | да | ISO 8601 с ТОЧКОЙ: `2026-08-05T22:07:26.753+03:00` |
| `storeFrom` | да | склад списания, на проде `8a55c561-2598-4150-ac28-1b03cf8a1835` |
| `expenseAccount` | да | счёт списания, на проде `97036ddb-b2e1-cd47-1669-c145daa9f9c5` |
| `items[].num` | да | порядковый номер строки |
| `items[].product` | да | GUID номенклатуры |
| `items[].amount` | да | количество |
| `items[].amountUnit` | **фактически да** | GUID единицы измерения |
| `items[].containerId` | **фактически да** | нулевой GUID, если контейнера нет |
| `comment`, `number`, `conception`, `isAutomatic`, `isEditable` | нет | необязательные |

### Ловушка: `amountUnit` и `containerId` схема объявляет необязательными, а API требует

Без них `create` отвечает **HTTP 500** с невнятным
`java.lang.RuntimeException: Exception while deserializing object WriteoffDocument[...]`.
GUID в этом сообщении — внутренний объект сервера, документ при этом НЕ создаётся (проверено
через `get`: 404). По тексту ошибки понять причину нельзя, поэтому: если `create` отдал 500,
первым делом сверять состав позиции с реальным актом через `get`.

### Что возвращает и в каком состоянии документ

`create` → `201` и `{"message": "...saved successfully", "documentId": ..., "documentNumber": ...}`.
Документ создаётся **НЕ проведённым**, поэтому в ОПиУ он не попадает: строка «Списание
продукции и сырья» берёт только проведённые. Проведение — отдельный вызов `post`.

`cancel` переводит документ в `status: DELETED` (`deleted=true`, `processed=false`) — он
остаётся видимым в `list`, но нигде не считается.

### Сдвиг времени: отправили 22:07 МСК, вернулось 19:07

Отправленный `2026-08-05T22:07:26.753+03:00` вернулся из `get` как
`2026-08-05T19:07:26.753+03:00` — offset сохранён, а само время сдвинуто на −3 часа. Для дня
это безразлично днём, но акт, созданный после 21:00 МСК, уедет **на предыдущие сутки**, а на
границе месяца — в предыдущий месяц. Дату создаваемого акта задавать серединой дня, а не
текущим временем.

### Rate limit

Cloud отвечает `429` при частых вызовах подряд: во время пробы `get` сразу после `cancel`
получил 429. Между вызовами нужна пауза либо ретрай с backoff.
